...
export IMAGE_ID=$(docker images | grep echoserver | grep latest | grep -v aws | awk '{print $3}'| uniq)
...
docker tag $IMAGE_ID ${REPO_NAME}/echoserver
docker push ${REPO_NAME}/echoserver
